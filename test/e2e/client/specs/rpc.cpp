...
static void test_raise(nq_rpc_t rpc, Test::Conn &tc) {
	...
	RPC(rpc, RpcType::Raise, msg.c_str(), msg.length(), ([done, code, msg](
		...
		done(r == code && memcmp(msg.c_str(), ((char *)data) + sizeof(int), msg.length()));
	}));	
}
...
static void test_notify(nq_rpc_t rpc, Test::Conn &tc) {
	...
	RPC(rpc, RpcType::NotifyText, text.c_str(), text.length(), ([done2](
		nq_rpc_t, nq_result_t r, const void *, nq_size_t) {
		done2(r >= 0);
	})));
}

static void test_server_stream(nq_rpc_t rpc, Test::Conn &tc) {
	...
	WATCH(tc, OpenStream, ([&tc, done2, done3, text](nq_rpc_t rpc2, void **ppctx) {
		TRACE("test_server_stream: stream creation");
		WATCH(tc, Request, ([rpc2, done3, text](
			...
			nq_rpc_reply(rpc3, 1, msgid, reply.c_str(), reply.length());
	...
	RPC(rpc, RpcType::NotifyText, text.c_str(), text.length(), ([done](
		nq_rpc_t, nq_result_t r, const void *, nq_size_t) {
		done(r >= 0);
	}));
}
