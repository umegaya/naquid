...
class NqBoxer {
 public:
  ...
  struct Op {
    ...
    Op(uint64_t serial, void *target_ptr, OpCode code, OpTarget target = OpTarget::Stream) : 
      serial_(serial), target_ptr_(target_ptr), code_(code), target_(target) { data_.len_ = 0; }
    ...
    inline void Poll(NqBoxer *p) {
      ...
      while (try_dequeue(op)) {
        ...
        case Conn: {
          ...
          p->InvokeConn(op->serial_, op->code_, c);
        } break;
        case Stream: {
        ...
        case Alarm: {
          ...
          case Start:
            ...
            break;
          case Finalize:
            p->InvokeAlarm(op->serial_, op->code_, a);
            break;
          default:
            ASSERT(false);
            break;
  ...
  //invoker
  inline void InvokeConn(uint64_t serial, OpCode code, NqSession::Delegate *unboxed) {
  ...
  inline void InvokeAlarm(uint64_t serial, OpCode code, NqAlarm *unboxed) {
    if (MainThread()) {
      if (unboxed->alarm_serial() == serial) {
  ...
  inline void InvokeStream(uint64_t serial, OpCode code, NqStream *unboxed) {
    if (MainThread()) {
      ...
      Enqueue(new Op(serial, unboxed, code));      
    }
  }
  inline void InvokeStream(uint64_t serial, OpCode code, 
                           const void *data, nq_size_t datalen, NqStream *unboxed) {
    if (MainThread()) {
      if (unboxed->stream_serial() == serial) {
        ASSERT(code == Send);
  ...
  inline void InvokeStream(uint64_t serial, OpCode code, 
                           ...
                           nq_size_t datalen, nq_closure_t on_reply, NqStream *unboxed) {
    if (MainThread()) {
      if (unboxed->stream_serial() == serial) {
        ASSERT(code == Call);
  ...
  inline void InvokeStream(uint64_t serial, OpCode code, 
                           uint16_t type, const void *data, 
                           nq_size_t datalen, nq_rpc_opt_t &rpc_opt, NqStream *unboxed) {
    if (MainThread()) {
      if (unboxed->stream_serial() == serial) {
        ASSERT(code == CallEx);
  ...
  inline void InvokeStream(uint64_t serial, OpCode code, 
                           uint16_t type, const void *data, nq_size_t datalen, NqStream *unboxed) {    
    if (MainThread()) {
      if (unboxed->stream_serial() == serial) {
        ASSERT(code == Notify);
